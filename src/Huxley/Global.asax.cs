...
namespace Huxley
{
    public class HuxleyApi : HttpApplication
    {
        protected void Application_Start()
        {
            ...
            // Stops the backing field names being used instead of the public property names 
            // (*Field & PropertyChanged etc.)
            GlobalConfiguration.Configuration.Formatters.JsonFormatter.SerializerSettings.ContractResolver =
                new CamelCasePropertyNamesContractResolver();

            // Returns JSON to the browser without needing to add application/json to the accept request header
            // - remove to use XML (becomes the default)
